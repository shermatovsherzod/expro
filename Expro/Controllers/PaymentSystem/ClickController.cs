...
namespace Expro.Controllers
{
    ...
    public class ClickController : BaseController
    {
        ...
        [HttpPost]
        public IActionResult prepare([FromForm] ClickRequestVM requestVM)
        {
        ...
        [HttpPost]
        public IActionResult complete([FromForm] ClickRequestVM requestVM)
        {
            ClickResponseVM responseVM = new ClickResponseVM();
            try
            {
        ...
        private ClickResponseVM checkComplete(ClickRequestVM requestDTO, ClickResponseVM responseDTO)
    {
        ...
        {
            ApplicationUser user = GetUserByAccountNumber(requestDTO.merchant_trans_id);
            bool checkResult = (user != null);
            if (!checkResult)
            {
        ...
        private void userUpdatePrepare(ClickRequestVM requestDTO, ClickResponseVM responseDTO)
        {
            ...
            ClickTransactionService.Add(transaction);
        ...
        private void userUpdateComplete(ClickRequestVM requestDTO, ClickResponseVM responseDTO)
        {
            ...
            ApplicationUser user = GetUserByAccountNumber(requestDTO.merchant_trans_id);
            UserBalanceService.ReplenishBalance(user, (int?)(requestDTO.amount) ?? 0);
            bool checkResult = true;
            if (checkResult)
            {
                ClickTransaction transaction = ClickTransactionService.GetByID((int?)(requestDTO.merchant_prepare_id) ?? 0);
        ...
        private ApplicationUser GetUserByAccountNumber(string accountNumber)
        {
            return _userManager.Users.FirstOrDefault(m => m.AccountNumber == accountNumber);
        }
    }
}
